    ...
    return docs.reduce((result,doc)=> (
        `${result}<div class="flex" id="${doc.action}_${doc.event}">
            ...
            <footer>
                ...
                <div>
                <button
                    onClick="emitEvent('${doc.event}',document.getElementById('${doc.event}_data').value)"
                    class='success'>
                        Emit
                </button>
                </div>
...
exports.generate_html_template = (emitLinks,results,version='2.3.1')=>{
    return `<!DOCTYPE html>
        <html lang="en">
            <head>
                <meta charset="UTF-8">
                ...
                <script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/${version}/socket.io.min.js"></script>
            </head>
            <body>
                <div class="flex two">
                    ...
                    <div class="full half-900">
                        <div class="three flex">
                            <input id="server" value="" class="two-third" type="text" placeholder="ws://" style="margin:0;">
                            <button onClick="toggleConnect()" id="connectBtn" class="sixth shyButton" style="margin:0;padding:0">Connect</button>
                            <button class="sixth pseudo button" style="margin:0;padding:0" onClick="document.getElementById('settingsForm').classList.toggle('showSettings')">Query</button>
                            <article class="card settingsForm" id="settingsForm">
                ...
                <footer class="flex full">
                    <p align="right">Powered by: <a href="https://github.com/cybercoder/socketio-doc">socketio-doc</a></p>
                </footer>
                ...
                <div id="consoleContainer" class="normalConsole">
                    <div id="consoleHead">
                        ...
                        <div>
                            <a href="javascript://">
                                <i 
                                    class="lni lni-eraser"
                                    style="color: #756af8;"
                                    onClick="document.getElementById('allEventsList').innerHTML=''"
                                ></i>
                            </a>
                            <a href="javascript://">
                                <i 
        ...
        <script>
            ...
            function toggleConnect() {
                ...
                socket.on("*",function(event,data) {
                    console.log(event);
                    let selecteds=[...document.querySelectorAll('input:checked')].map(e => e.value);
                    ...
                    li.innerHTML="\
                        <div style='display:flex;flex-direction:row;align-items: center;'>\
                            <div style='flex:0;text-align: center;'>\
                                ...
                                <h3><span class='label' style='margin:0'>"+event+" received</span></h3>\
                            </div>\
                            <div style='flex:1;padding:5px;'>\
                                <code>"+(typeof data === 'object' ? JSON.stringify(data,null,2) : data)+"</code>\
                            </div>\
                        </div>\
                    "
            ...
            function emitEvent(ev,d) {
                ...
                li.innerHTML="\
                    <div style='display:flex;flex-direction:row;align-items: center;'>\
                        <div style='flex:0;text-align: center;'>\
                            <p style='font-size:10px;'>"+new Date().toLocaleString()+"</p>\
                            <h3><span class='label' style='margin:0'>"+ev+" emitted</span></h3>\
                        </div>\
                        <div style='flex:1;padding:5px;'>\
                            <code>"+(typeof data === 'object' ? JSON.stringify(data,null,2) : data)+"</code>\
                        </div>\
