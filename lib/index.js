...
exports.extracted_docs_to_html = docs =>{
    if(docs[0].action == "emit"){
        ...
        return docs.reduce((result,doc)=> (` <input type="checkbox" id="${doc.action}_${doc.event}" name="${doc.event}" value="${doc.event}"><label for="${doc.action}_${doc.event}">${doc.event}</label><br>`),'');
